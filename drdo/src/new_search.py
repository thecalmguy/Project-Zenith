...
class UAV:
    ...
    #Function to mark searched areas
    def mark_map(self):
        global global_map
        #Points to mark

        global_map[int(3*self.curr_pose.position.x+20), int(3*self.curr_pose.position.y+50)] = 1
...
if __name__ == "__main__":
    ...
    uav2 = UAV(2)
    #Search
    global_map = np.zeros([80,150], dtype=int)
    ...
    plt.matshow(global_map)
    plt.show(block = False)
    while True:
        plt.clf()
        plt.matshow(global_map, fignum=0)
        plt.show(block = False)
        plt.pause(0.001)
        if zam_search_completed: break
    #Spin
